...
COMMON_ARGS="--parsable --nodes=$nodes --gpus-per-node=8 --account=llmservice_fm_post --job-name=nemosftmix --partition=batch --time=240 --cpus-per-task=128 --mem=0 --output=logs/nemosft_%j/%a.out --error=logs/nemosft_%j/%a.err"
...
command="${starting_command} && uv run examples/run_sft.py --config=examples/configs/sft_acetoolreason_mix_megatron.yaml cluster.num_nodes=$nodes"
